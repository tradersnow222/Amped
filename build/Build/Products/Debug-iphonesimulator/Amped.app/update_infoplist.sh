...
if [ -f "$INFO_PLIST_PATH" ]; then
    ...
    if /usr/libexec/PlistBuddy -c "Print :NSHealthUpdateUsageDescription" "$INFO_PLIST_PATH" &>/dev/null; then
        ...
        /usr/libexec/PlistBuddy -c "Set :NSHealthUpdateUsageDescription 'For technical reasons, this permission appears but Amped does not modify or write any data to Health. We only read your health data to provide insights.'" "$INFO_PLIST_PATH"
    else
        echo "Adding NSHealthUpdateUsageDescription"
        /usr/libexec/PlistBuddy -c "Add :NSHealthUpdateUsageDescription string 'For technical reasons, this permission appears but Amped does not modify or write any data to Health. We only read your health data to provide insights.'" "$INFO_PLIST_PATH"
    fi
