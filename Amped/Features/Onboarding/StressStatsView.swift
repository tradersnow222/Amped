...
struct StressStatsView: View {
    ...
    enum StressLevel: String, CaseIterable {
        ...
        var subtitle: String {
            switch self {
            case .low:
                return "(rarely feel stressed)"
            case .moderate:
                return "(Occassionally stressed)"
            case .high:
                return "(Constantly stressed)"
            }
        }
    }
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            if isFromSettings {
                let saved = appState.getFromUserDefault(key: UserDefaultsKeys.userStressLevel)
                if !saved.isEmpty {
