...
@MainActor final class HealthKitDataManager {
    ...
    func fetchLatestData(for metricType: HealthMetricType) async -> HealthMetric? {
        ...
        if metricType == .sleepHours {
            return await sleepManager.processSleepData(from: Calendar.current.date(byAdding: .day, value: -7, to: Date())!, to: Date())
        }
        ...
        do {
            ...
            let value = sample.quantity.doubleValue(for: metricType.unit ?? HKUnit.count())
            return HealthMetric(
                id: UUID().uuidString,
                type: metricType,
                value: value,
