...
struct MetricChartSection: View {
    ...
    private var mainChartView: some View {
        ...
        return Chart {
            ...
            // negative (red)
            ForEach(segments.neg, id: \.self) { seg in
                SegmentAreaLine(segment: seg, color: .red)
            }

            // positive (green)
            ForEach(segments.pos, id: \.self) { seg in
                SegmentAreaLine(segment: seg, color: .green)
            }
...
//            plot.background(Color.white.opacity(0.05))
//        }
        ...
        var body: some ChartContent {
            ForEach(segment) { p in
                LineMark(
                    ...
                    series: .value("seg", color == .green ? "pos" : "neg")
                )
...
//    // MARK: - Main Chart
//    
//    private var mainChartView2: some View {
//        // --- COLORED LINE (must be segmented to support two colors) ---
//        let segments = splitSegmentsBySign(chartData)
//        return Chart {
//            // --- AREA BACKGROUND (green above 0, red below 0) ---
////            ForEach(Array(chartData.enumerated()), id: \.offset) { idx, p in
////                AreaMark(
////                    x: .value("Time", idx),
...
////                .foregroundStyle(
////                    p.value >= 0 ?
////                        LinearGradient(colors: [Color.green.opacity(0.35), .clear],
////                                       startPoint: .top, endPoint: .bottom)
////                    :
////                        LinearGradient(colors: [Color.red.opacity(0.35), .clear],
////                                       startPoint: .top, endPoint: .bottom)
////                )
////            }
//            
//            // --- SINGLE LINE (colored point-wise) ---
////            ForEach(Array(chartData.enumerated()), id: \.offset) { idx, p in
////                LineMark(
////                    x: .value("Time", idx),
////                    y: .value("Delta", p.value)
////                )
////                .foregroundStyle(p.value >= 0 ? Color.green : Color.red)
...
//
//            // --- SELECTION LINE / POINT ---
//            if let idx = selectedIndex, idx < chartData.count {
